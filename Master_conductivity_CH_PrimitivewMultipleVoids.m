...
if(flagsupercell == 1)
    ...
    YY = 0; % ((0:1/N:1-1/N)+1/N/2)*1*pi/A1; %(-1:1/32:1)*1*pi/(A1); %(-1:0.0625:1)*1*pi/(A1); % 0.0*kxx;% 
    ZZ = 0; % ((0:1/N:1-1/N)+1/N/2)*1*pi/A1; %(-1:1/32:1)*1*pi/(A1); % (-1:0.0625:1)*1*pi/(A1); % 0.0*kxx;% (-1:0.0667:1)*1*pi/(A1); % 0.0*kxx; 
nbranch = nsupercell*3; % length(xs)*3; % ncell*nbasis*(ncxsuper^3*3 - ncxvoid);

else 
    XX = ((0:1/N:1-1/N)+1/N/2)*2*pi/A1; %
    YY = 0; % ((0:1/N:1-1/N)+1/N/2)*2*pi/A1; %(-1:1/32:1)*1*pi/(A1); %(-1:0.0625:1)*1*pi/(A1); % 0.0*kxx;% 
    ZZ = 0; % ((0:1/N:1-1/N)+1/N/2)*2*pi/A1; %(-1:1/32:1)*1*pi/(A1); % (-1:0.0625:1)*1*pi/(A1); % 0.0*kxx;% (-1:0.0667:1)*1*pi/(A1); % 0.0*kxx; 
...
dky = 0; % (YY(2) - YY(1))/sigmaSi; 
dkz = 0; % (ZZ(2) - ZZ(1))/sigmaSi; 
...
%             clear checkeV M;
%     checkeV = []; M = []; 
    vmode = (wwI - wwf(ip))*sqrt(1/massSi)/(1*0.01*1*pi/A1/sigmaSi);
