...
%	fflush(stdout)
	disp(nk/length(kxx));
                ...
                if (kyy(nk)>kxx(nk) || kzz(nk)>kxx(nk))
                   continue;
                else if (kyy(nk) == kxx(nk) && kzz(nk) == kxx(nk))
                    w = 1;
                    wx = 1; wy = 0; wz = 0;
                    else if(kyy(nk) == kxx(nk))
                         
                        w = 2;
                        wx = 1; wy = 0; wz = 1;
                        else if(kzz(nk) == kxx(nk))
...
% % %  % % %### Velocity #1: Dispersion Values at a smaller forward. 
...
% % %  % % %### END Velocity #1: Dispersion Values at a smaller forward. 
...
% % %     vmode(nz,:) = diag(eV'*((Dsfx-Dsuper)*(1/massSi)/(dkx/sigmaSi))*eV)./(2*wwI*sqrt(1/massSi));
    vmode = diag(eV'*((Dsfx-Dsbx)*(1/massSi)/(2*dk/sigmaSi))*eV)./(2*wwI*sqrt(1/massSi));
...
% %     kmode_holland = vmode.*vmode.*tot_scat.*Cpmode; %  + wy(nk)*vmodey.*vmodey.*tot_scat.*Cpmode + wz(nk)*vmodez.*vmodez.*tot_scat.*Cpmode; %
    % % % *** Velocity #3 Equation-Part BZ-Tetrahedron 
    kmode_holland = wx*vmode.*vmode.*tot_scat.*Cpmode + wy*vmodey.*vmodey.*tot_scat.*Cpmode + wz*vmodez.*vmodez.*tot_scat.*Cpmode; %
