...
flagsupercell = 0; % % 1: supercell; 0: primitive cell. 
if(flagsupercell == 1)
    A1 = A*ncxsuper; % *ncx; % ; %
else 
    A1 = A*1;
...
% kindex = nxmax;
% nbranch = 3*ncell*nbasis*(ncxsuper^3 - ncxvoid); 
[Phiall, rxall, ryall, rzall]=dispersioncurveSuperCell_Modify(A,ncell,rc,Ksp,naxis,mI,nbasis,forceflag,fmat,xs,ys,zs,ncx,ncy,ncz,ncxsuper,nsupercell);  
...
kB = 1.3806488e-23;
% % % *****C-H model Relaxation time approxmiation from Davis or Hopkins. 
B = 2.1e-19; 
C = 180;
D = 1.32e-45;
n=0;
...
% % % ****Rexlaxation time approximation from 1st principle (Ward & Broido, 2011)
AN_TA = 253322; % units: (meV^2*K*s)^(-1);
AN_LA = 163921;
AU_TA = 2012;
AU_LA = 509; 
% meV2J = 1.6021773e-13; % 1meV = 1.6021773e-13J. 
% J2meV = 1/meV2J;
J2meV = 6.2415093433e+18*1e3;
meV2J = 1/J2meV;
thetaD = 645; % unit: K. From google "What's Debye temperature for silicon"
gT = T*(1-exp(-3*T/thetaD)); % single function, thetaD=Debye temp. From (Ward & Broido, 2011).
...
%             clear checkeV M;
%     checkeV = []; M = []; 
    vmode = (wwI - wwf(ip))*sqrt(1/massSi)/(1*0.0001*1*pi/A1/sigmaSi);
...
%       wwmode = sort(wwI)*sqrt(1/massSi); 
%         end
  
% % % %     % % %**** k with Relaxation time from C-H model of Davis or Hopkins    
...
% % % %     ww(nk,:) = wwmode;
% % % %     vv(nk,:) = vmode;
% % % %     % % %**END** k with Relaxation time from C-H model of Davis or Hopkins   
    
        % % %**** k with Relaxation time from 1st principle-Ward&Broido(2011) 
        clear wwmode;
    [wwmode,bb] = sort(wwI*sqrt(1/massSi));
    vmode = vmode(bb);
        ww(nk,:) = wwmode;
    vv(nk,:) = vmode;
    
         % % % **** Cpmode from 1st principle, (Ward&Broido,2011)
    n_bose = 1./(exp(h*wwmode/kB/T) - 1);
    Cpmode = kB*(h*wwmode/kB/T).^2.*n_bose.*(n_bose+1); % (exp(h*wwmode/kB/T)./((exp(h*wwmode/kB/T)-1).^2));
    Cpsum = Cpsum + sum(sum((Cpmode))); %
% % %     TN_TA1 = 1/(AN_TA*meV2J^(2)*wwmode(1)^2*gT); % TA-1 or TA-2;
% % %     TU_TA1 = 1/(AU_TA*meV2J^(4)*wwmode(1)^4*gT);
% % %     TN_TA2 = 1/(AN_TA*meV2J^(2)*wwmode(2)^2*gT); % TA-1 or TA-2;
% % %     TU_TA2 = 1/(AU_TA*meV2J^(4)*wwmode(2)^4*gT);
% % %     TN_LA = 1/(AN_LA*meV2J^(2)*wwmode(3)^2*gT); % TA-1 or TA-2;
% % %     TU_LA = 1/(AU_LA*meV2J^(4)*wwmode(3)^4*gT);
    TN_TA1 = 1/(AN_TA*(wwmode(1)*h*J2meV)^2*gT); % TA-1 or TA-2;
    TU_TA1 = 1/(AU_TA*(wwmode(1)*h*J2meV)^4*gT);
    TN_TA2 = 1/(AN_TA*(wwmode(2)*h*J2meV)^2*gT); % TA-1 or TA-2;
    TU_TA2 = 1/(AU_TA*(wwmode(2)*h*J2meV)^4*gT);
    TN_LA = 1/(AN_LA*(wwmode(3)*h*J2meV)^2*gT); % TA-1 or TA-2;
    TU_LA = 1/(AU_LA*(wwmode(3)*h*J2meV)^4*gT);
    
% % %     TRTA_TA = ((AN_TA*meV2J^(2)*wwmode(1)^2*gT) + (AU_TA*meV2J^(4)*wwmode(1)^4*gT))^(-1);
% % %     TRTA_LA = ((AN_LA*meV2J^(2)*wwmode(3)^2*gT) + (AU_LA*meV2J^(4)*wwmode(3)^4*gT))^(-1); 
    
    TRTA_TA1 = (TN_TA1^(-1) + TU_TA1^(-1))^(-1);
    TRTA_TA2 = (TN_TA2^(-1) + TU_TA2^(-1))^(-1);
    TRTA_LA = (TN_LA^(-1) + TU_LA^(-1))^(-1);
    
    TRTA = [TRTA_TA1;TRTA_TA2;TRTA_LA;0;0;0]; 
    kmode_holland = vmode.*vmode.*TRTA.*Cpmode; %
     
    ksumx2(nk) = sum(sum(kmode_holland));

    % % %**END**k with Relaxation time from 1st principle-Ward&Broido(2011) 
    
% % % %     % % % **** Cpmode from C-H model, (Davis or Hopkins)
...
k1 = ksum*dkx*dky*dkz/(2*pi)^3 %  % Primitive Cell Only  
...
% % % % csvwrite('ksumx2-ncx6-ncxvoid2-Method1-N10.xlsx',ksumx2)
% % % % csvwrite('ww-ncx6-ncxvoid2-Method1-N10.xlsx',ww)
% % % % csvwrite('vv-ksumx2-ncx6-ncxvoid2-Method1-N10.xlsx',vv)
