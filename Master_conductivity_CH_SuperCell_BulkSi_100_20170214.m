...
atype = ones(1,na); 
% % mI = massGe/massSi.*ones(1,na); % % ones(1,nsupercell);
% % atype = 2.*ones(1,na); % % ones(1,nsupercell);
...
% % % C = 137.3; % unit: K. 
% % % D = 1.32e-45; % s^3. % % % N = 30; 
Nk = 80; % 50; % 30; %  % 30; %50; %120; % 120; %    50; %    80; % 30; %  60; %80; %   30; % 120; % 50; %[80]; % 30; %  [10, 20, 30, 40, 50, 80, 100]; % ]; %      
...
ksumtest2 = zeros(length(kxx),1);
ksumtest3 = zeros(length(kxx),1);
ksumtest4 = zeros(length(kxx),1);
...
%         end
    ...
    umk_scat_inv = B*(wwmode.*wwmode)*T*exp(-C/T); %<== Correct Tao_U.  % B*(wwmode.*wwmode)./(sinh(h*wwmode/kB/T)); %test#2 % B*(wwmode)*T*exp(-C/T); % test#1 Tao_U. %     
    umk_scat_inv2 = B*(wwmode)*T*exp(-C/T); %
    umk_scat_inv3 = B*(wwmode.*wwmode)./(sinh(h*wwmode/kB/T));
    umk_scat_inv4 = (h*gamma^2*T/massSi/theta*exp(-theta/3/T))*(wwmode.*wwmode)./(vmode.*vmode); % 
    % impurity scattering. 
    ...
    kmode_holland2 = vmode.*vmode.*Cpmode.*(1./(umk_scat_inv2 + imp_scat_inv  + bound_scat_inv));
    kmode_holland3 = vmode.*vmode.*Cpmode.*(1./(umk_scat_inv3 + imp_scat_inv  + bound_scat_inv));
    kmode_holland4 = vmode.*vmode.*Cpmode.*(1./(umk_scat_inv4 + imp_scat_inv  + bound_scat_inv));
    %     kmode_holland = vmode.*vmode.*1.*Cpmode; %
    
    ksum = ksum + sum(sum(kmode_holland*(kall/sigmaSi)^2));
    ksumtest2(nk) = sum(sum(kmode_holland2*(kall/sigmaSi)^2));
    ksumtest3(nk) = sum(sum(kmode_holland3*(kall/sigmaSi)^2));
    ksumtest4(nk) = sum(sum(kmode_holland4*(kall/sigmaSi)^2));
% % %     ksumx(nk) = ksum;
...
% % %     ksumx2(nk) = ksum2;
    ...
    aaa = find(vmode == 0);
    vmode(aaa) = (abs(vmode(aaa-1)) + abs(vmode(aaa+1)))/2; 
...
k = ksum*(dk)/(6*pi^2) ;
ktest2 = sum(ksumtest2)*(dk)/(6*pi^2); 
ktest3 = sum(ksumtest3)*(dk)/(6*pi^2) ;
ktest4 = sum(ksumtest4)*(dk)/(6*pi^2) ;
[k,ktest2,ktest3, ktest4]
