...
% % % C = 137.3; % unit: K. 
% % % D = 1.32e-45; % s^3. % % % N = 30; 
Nk = 200; % 20; % 50; % 30; %  % 30; %50; %120; % 120; %    50; %    80; % 30; %  60; %80; %   30; % 120; % 50; %[80]; % 30; %  [10, 20, 30, 40, 50, 80, 100]; % ]; %      
...
% % % % % ### GROUP VELOCITY ***Central***######## 
    ...
    dw = abs(wwb - wwf(ip)).*sqrt(1/massSi);
    wwI = (wwb + wwf(ip))/2;
    vmode = vmodec;
% % % ### GROUP VELOCITY ***Central***######## 
...
%     kmode_holland = vmode.*vmode.*1.*Cpmode; %
    kmode_holland_w = wwmode.*wwmode./vmodec.*tot_scat.*Cpmode.*dw; %
    
    ksum = ksum + sum(sum(kmode_holland*(kall/sigmaSi)^2));
    ksumw = ksumw + sum(sum(kmode_holland_w));
...
% % %     ksumx2(nk) = ksum2;
   ...
   %  Uprocess_scat_inv = bu*(wwmode.*wwmode)./(vmode.*vmode);
    gru =  0.73; theta = 373; % Ge % Si: gamma =  1.5; theta = 674; % % 645; % Debye temperature, unit: K; % Glassbrener: Si = 674; % Website: Ge: 373;
    massAve = sum(mI)/sum(length(mI))*massSi;
		   Uprocess_scat_inv = (h*gru^2*T/massAve/theta*exp(-theta/3/T))*(wwmode.*wwmode)./(vmode.*vmode);
...
%     tot_scat_Gillet = 1./tot_scat_inv_Gillet;
    % % %
    if(mod(nk,1000) == 0)
        disp('Umklapp from Bulk Si - Davis-2011 paper')
    end
    tot_scat_inv_Gillet = umk_scat_inv + inc_scat_inv; % 1; % 
    tot_scat_Gillet = 1./tot_scat_inv_Gillet;
