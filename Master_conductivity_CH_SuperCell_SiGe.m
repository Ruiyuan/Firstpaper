...
% % % C = 137.3; % unit: K. 
% % % D = 1.32e-45; % s^3. % % % N = 30; 
Nk = [30]; %48; %  60; % [ 48]; % 80]; % 72]; % 36, 48 , 60]; % ]; % ,36, 48 , 60,  80, 96, 120, 160];    
...
fileID = fopen('Fig1_BulkSi_N4_nk_16_ksum.csv','w'); % 'ncx','N');
...
% %     clear Dall UU SS VV; 
    Dsuper = zeros(naxis*nsupercell, naxis*nsupercell);
    kxf = (kx + 0.001*1*pi/(A1)); %(kx+0.05*kx); % (kx + 0.025*1*pi/(A1)); %(kx+0.01); %  
...
%             clear checkeV M;
%     checkeV = []; M = []; 
    vmode = (wwI - wwf(ip))*sqrt(1/massSi)/(1*0.001*1*pi/A1/sigmaSi); %(1*0.05*kx/sigmaSi); %  (1*0.01/sigmaSi); % 
...
Nresult(ink,:) = [Nk(ink),Cp3,k3,k_NBS,k_Particle,timevalue];
% Cp3 = 0; Cpsum = 0; tic

end

% xlswrite('Fig1_BulkSi_N4_nk_16.xlsx',Nresult)
% 
