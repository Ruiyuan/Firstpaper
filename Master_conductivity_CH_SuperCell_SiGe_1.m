...
% %     clear Dall UU SS VV; 
    Dsuper = zeros(naxis*nsupercell, naxis*nsupercell);
    kxf = (kx +0.025*1*pi/(A1)); % (kx+0.0005); % (kx +0.01*1*pi/(A1));
...
% % % % % %             clear checkeV M;
% % % % % %     checkeV = []; M = []; 
% % % % %     vmode = (wwb - wwf(ip))*sqrt(1/massSi)/(1*0.01*1*pi/A1/sigmaSi); (2*0.001*1*pi/A1/sigmaSi);
% % % % % % % % % % ### GROUP VELOCITY ***BACKWARD***######## 
...
%             clear checkeV M;
%     checkeV = []; M = []; 
    vmode = (wwI - wwf(ip))*sqrt(1/massSi)/(1*0.025*1*pi/A1/sigmaSi); % (1*0.0005/sigmaSi); % 
