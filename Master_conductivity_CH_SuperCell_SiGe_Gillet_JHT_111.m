...
% %     clear Dall UU SS VV; 
    Dsuper = zeros(naxis*nsupercell, naxis*nsupercell);
    kxf = (kx+0.001*1*pi/A1); % (kx + 0.025*1*pi/(A1)); %(kx+0.01); %  
    kyf = ky+0.001*1*pi/A1; % kxf; 
    kzf = kz+0.001*1*pi/A1; % kxf; 
...
% % %             clear checkeV M;
% % %     checkeV = []; M = []; 
% %     vmodebf = (wwb - wwf(ip))*sqrt(1/massSi)/(2*0.001*1*pi/A1/sigmaSi);
% % % % % % % ### GROUP VELOCITY ***BACKWARD***######## 
...
%     checkeV = []; M = []; 
    ...
    vmode = (wwI - wwf(ip))*sqrt(1/massSi)/(1*dkxvg/sigmaSi); % (1*0.025*1*pi/A1/sigmaSi); % (1*0.01/sigmaSi); % 
% %     wwI = (wwI + wwf(ip))/2;
...
% % % % % % % % 
% % % % % % % % % %% 
% % % % % % % % k_NBS = sum(sum(ksumx_NBS))*8/V
% % % % % % % % k_Particle = sum(sum(ksumx_3Part))*8/V
% % % % % % % % 
% % % % % % % % k_Gillet = sum(sum(ksumx_4Gillet))*8/V
% % % % % % % % % % % Subroutine: End
...
% save('Fig3_QD')
% xlswrite('Fig3_SiGeQD_Gillet_111_nk102030.xlsx',Nresult)
