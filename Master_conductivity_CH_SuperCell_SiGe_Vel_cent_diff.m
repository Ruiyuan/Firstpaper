...
% % % C = 137.3; % unit: K. 
% % % D = 1.32e-45; % s^3. % % % N = 30; 
Nk = [30]; %; %  ; % [ 48]; % 80]; % 72]; % 36, 48 , 60]; % ]; % ,36, 48 , 60,  80, 96, 120, 160];    
...
% % NP = [1265:3456, 5043:6912, 7953:10368, 11399:13824]; % FOR ncx_4_Nk_24 ONLY. length(NP)= 8904. 
fileID = fopen('Fig3_QDGillet_N5_nk_25_ksum_Vel_cent_diff_P8_4985_5000.csv','w'); % 'ncx','N')

% parfor nk = 1:length(kxx);
tic
for nk = 1:length(kxx); % NP(7658:8000); % NP(1931:2000, 4670:5000, 5706:6000, 7658:8000, 8849:length(NP)); % 1:length(kxx); % NP(8601:length(NP)); % NP(1710:2000); % NP(758:1000); % NP(8001:length(NP)); % [1:5]; % %  ; % 1+length(kxx)/4*3:length(kxx)/4*4; %
...
% %     clear Dall UU SS VV; 
    Dsuper = zeros(naxis*nsupercell, naxis*nsupercell);
    kxf = (kx +0.001*1*pi/(A1)); %  kxf = (kx+0.05*kx); % (kx + 0.025*1*pi/(A1)); %(kx+0.01); %  
...
% xlswrite('Fig1_BulkSi_N5_nk_16_Vel_cent_diff_P5.xlsx',Nresult)
% 
...
plot(N2*2,K2,'-yo','LineWidth',2,'MarkerSize',8)
