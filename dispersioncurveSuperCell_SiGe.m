function [Phiall, rxall, ryall, rzall]=dispersioncurveSuperCell_SiGe(A,ncell,rc,Ksp,naxis,mm,nbasis,forceflag,fmatSiGeQD,xs,ys,zs,ncx,ncy,ncz,ncxsuper,nsupercell)
% kv = kvI; mm = mI ; fmatSiGeQD = fmatSi; 
...
% % % % %    Dsuper = zeros(naxis*nsupercell,naxis*nsupercell);
 ...
 nbasis=4;  
 % % % ********Sub: SuperCell Calculation for Dynamical Matrix use fmatSiGeQD
 % ***********% % %
    for i=1:nsupercell; 
...
% % % % % %                            if(-1<=dzij & dzij<=1)
% % % % % %                                
% % % % % % %                                    Phi=fmatSiGeQD((i-1)*3+1:i*3, (jprime-1)*3+1:jprime*3, dxij+2, dyij+2, dzij+2);
% % % % % %                                    Phi=fmatSiGeQD((i1primy-1)*3+1:i1primy*3, (j1primy-1)*3+1:j1primy*3, dxij+2, dyij+2, dzij+2);
% % % % % %                                
% % % % % %                            end
% % % % % %                        end
% % % % % %                      end
