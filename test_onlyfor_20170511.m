...
% % % C = 137.3; % unit: K. 
% % % D = 1.32e-45; % s^3. % % % N = 30; 
Nk = 128; % 200; % 50; % 30; %  % 30; %50; %120; % 120; %    50; %    80; % 30; %  60; %80; %   30; % 120; % 50; %[80]; % 30; %  [10, 20, 30, 40, 50, 80, 100]; % ]; %      
...
if(flagsupercell == 1)
    XX = ((0:1/N:1-1/N)+1/N/2)*1*pi/(A1*sqrt(2)); % *1*pi/A1; %  (0.001:0.0625:1)*1*pi/(A1); 
...
ksumx_test_100 = zeros(length(kxx),1);
...
% % %         vmodez = zeros(nbranch);
    ...
    kx=kxx(nk);
    ky= kx; % XX(1); % kx; %  kx; % kx; %  0.46*kx; %  kx; % kx; % 0; % kyy(nk);
    kz= XX(1); % kx; % 0.79*kx; % 0; % kx; % kx; % 0; % kzz(nk);
...
%     kmode_holland = vmode.*vmode.*1.*Cpmode; %
    ...
    ksum = ksum + sum(sum(kmode_holland*(kall/sigmaSi)^2));
        ksumx_test_100(nk) = sum(sum(kmode_holland*(1/kall)^2));
...
k_Iso_2 = ksum*dk*(4/3*(ncx)^3/pi^2)
...
ktest = sum(sum(ksumx_test_100))*2*Nk*Nk/dk/(pi^2);
...
V = 1.77154066392053e-20;
sum(ksumx2)*8/V
kDavis110 = ksum*dk*4*pi/3/((pi/sqrt(2))^3)/4/2
% sum(Ave_kGillet_bin)
