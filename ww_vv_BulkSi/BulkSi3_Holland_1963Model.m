...
vv = zeros(nbranch, N^3);
    ...
    for Pendflag1 = 1:length(Pendflag);
        ...
        fname31 = sprintf('ww_BulkSi%d_Nk_%d_P%d_%d.csv', ncx, N, Pendflag1,Ptotal);
        ww(:,kstart:kend) = load(fname31,'-ascii');
        fname32 = sprintf('vv_BulkSi%d_Nk_%d_P%d_%d.csv', ncx, N, Pendflag1,Ptotal);
...
kT0_sum = zeros(length(kxx),1);
kL_sum = zeros(length(kxx),1);
kT0_mode = zeros(nbranch,1);
kL_mode = zeros(nbranch,1);
kholland_sum = zeros(length(kxx),1);
...
%     		   disp('use kmode_holland for Bulk Si Calculation: ')
    ...
    % % % --- Holland model----&&&
    x = h*wwmode/kB/T;
    CT = (kB/(2*pi^2*vT))*(kB/h)^3;
    CL = (kB/(2*pi^2*vL))*(kB/h)^3;
    dx = theta3/200;
    for ii = 1:nbranch;
        xii = x(ii);
        if(x(ii)<=theta1/T)
            kT0_mode(ii) = CT.*(xii^4)*exp(xii)*((exp(xii)-1)^(-2))/(vb/L/F + alpha*xii^4*T^4 + betaT*xii*T^5)*dx;
        else if(x(ii)<=theta2/T)
                kT0_mode(ii) = CT.*(xii^4)*exp(xii)*((exp(xii)-1)^(-2))/(vb/L/F + alpha*xii^4*T^4 + betaTU*(xii^2)*T^2/(sinh(xii)))*dx;
            else 
                kT0_mode(ii) = 0;
            end
        end
        if(x(ii)<=theta3/T)
            kL_mode(ii) =  CL.*(xii^4)*exp(xii)*((exp(xii)-1)^(-2))/(vb/L/F + alpha*xii^4*T^4 + betaL*(xii^2)*T^5)*dx;
        else
            kL_mode(ii) = 0;
        end
    end
    kT0_sum(nk) = sum(kT0_mode)*T^3*2/3;
    kL_sum(nk) = sum(kL_mode)*T^3/3;
    kholland_sum(nk) = sum(kT0_mode)*T^3*2/3 + sum(kL_mode)*T^3/3;
%     Cp_holland(nk) = sum(sum(Cp));
    % % % END: Holland model. % % % 
...
% % %            Taosumx_incINV(nk) = sum(sum((inc_scat_inv)))*8/V;
           ...
           tao_1 = tot_scat; % tot_scat_Gilletc; 
...
[k_110SBZ, k_110All, k_111SBZ, k_111All]

k_holland = sum(kT0_sum) + sum(kL_sum)
kholland_SBZ = sum(kT0_sum(kpos_SBZ(1:np_SBZ))) + sum(kL_sum(kpos_SBZ(1:np_SBZ)))
sum(kholland_sum(kpos_SBZ(1:np_SBZ)))
np_SBZ/length(kxx)
