...
tao_umk_sum = zeros(length(kxx),1);
tao_I_sum = zeros(length(kxx),1);
tao_Uprocess_sum = zeros(length(kxx),1);

wmode_sum = zeros(length(kxx),1);
w2mode_sum = zeros(length(kxx),1);
w4mode_sum = zeros(length(kxx),1);
...
% 		   Cpsumnkc(nk) = sum(sum((Cpmodec)));
   ...
   %  Uprocess_scat_inv = bu*(wwmode.*wwmode)./(vmode.*vmode);
    gamma = 1.5; theta = 674; % Debye temperature, unit: K
...
% 		   kmode_holland4_Gilletc = k_reduced.*tot_scat_Gilletc; % vmode.*vmode.*tot_scat_Gilletc.*Cpmodec; %
           if(mod(nk,1000) == 0)
            disp('Umklapp use Si/Ge QDSL - Davis-2011: w2/v2')
           end
           % tot_scat_inv = Uprocess_scat_invc + imp_scat_inv  + bound_scat_inv; % ;%
            tot_scat = 1./tot_scat_inv;
            kmode_holland = vmode.*vmode.*tot_scat.*Cpmode; %
...
% % %         tao_4 = 1; 
% % %     end
    wmode_sum(nk) = sum(wwmode);
    w2mode_sum(nk) = sum(wwmode.*wwmode);
    w4mode_sum(nk) = sum(wwmode.*wwmode.*wwmode.*wwmode);
    tao_1_sum(nk) = sum(tao_1); % sqrt(tao_1'*tao_1);
        tao_Uprocess_sum(nk) = sum(1./Uprocess_scat_invc); % 
                tao_umk_sum(nk) = sum(1./umk_scat_inv); % 
            tao_I_sum(nk) = sum(1./imp_scat_inv); % sqrt(tao_1'*tao_1);
