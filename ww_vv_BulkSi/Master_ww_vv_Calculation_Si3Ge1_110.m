...
Nk = 80; % 120; % 50; %120; % 60; %     80; %20; % 100; % 16; %40; %   [60]; %   
...
Ptotal = 50; % 125; % 80; % 20; % 
Pendflag = [2:5]; %[9:11]; %[7,8]; % [33:34]; %[31:32]; %[28:30]; % [25:27]; %[24]; %[21:23]; %[18:20]; %[16:17]; %Si3Ge1: [11:15]; %[1:10]
mypool = parpool(4);
...
wwtotal111 = zeros(nbranch, length(kxx));
vvtotal111 = zeros(nbranch, length(kxx));
...
% % % %      vmode111 = (wwb - wwf(ip))*sqrt(1/massSi)/(1*dkxvg/sigmaSi);
% % % % 		   wwc111 = (wwb+wwf(ip))/2;
% % % % 		   wwmodec111 = wwc111*sqrt(1/massSi);
% % % %      eVf = []; eVb = [];  
% % % % 
% % % % 
% % % % %%%%%%%-----END----------------------------------------------------------
...
%     vv(:,nk-kstart+1) = vmode;
    wwtotal(:,nk) = wwmode;
    vvtotal(:,nk) = vmode;
% % %     wwtotal111(:,nk) = wwmodec111;
% % %     vvtotal111(:,nk) = vmode111;
...
% % %     ww111(:,1:bandwidth) = wwtotal111(:,kstart:kstop);
% % %     vv111(:,1:bandwidth) = vvtotal111(:,kstart:kstop);
%     cd C:\Si3Ge1_110
    fname31 = sprintf('ww_Si%dGe1_Nk_%d_P%d_%d_110.csv', ncx, N, Pn(end),Ptotal);
    save(fname31,'ww','-ascii');
    fname32 = sprintf('vv_Si%dGe1_Nk_%d_P%d_%d_110.csv', ncx, N, Pn(end),Ptotal);
    save(fname32,'vv','-ascii');
% % %     fname33 = sprintf('ww_Si%dGe1_Nk_%d_P%d_%d_111.csv', ncx, N, Pn(end),Ptotal);
% % %     save(fname33,'ww111','-ascii');
% % %     fname34 = sprintf('vv_Si%dGe1_Nk_%d_P%d_%d_111.csv', ncx, N, Pn(end),Ptotal);
% % %     save(fname34,'vv111','-ascii');
%     fclose(fileID);
