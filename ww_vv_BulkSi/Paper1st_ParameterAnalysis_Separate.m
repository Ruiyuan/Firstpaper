...
v2tao_1_bin = zeros(length(Nk),nbins);
v2_bin = zeros(length(Nk),nbins);
tao_1_bin = zeros(length(Nk),nbins);
...
disp(nbranch)
ninc = 0; 
for nk = 1: length(kxx); % kstart:kstop; 
...
%       fprintf(fileID,'%6d %12.8f %12.8f %12.8f %12.8f %12.8f\n', nk, Cpsumnk(nk),ksumx2(nk), ksumx_NBS(nk), ksumx_3Part(nk),ksumx_4Gillet(nk));
   ...
   %     for i = 1:nbranch; 
        ...
        v2_bin(nk,binIdx(1:nbranch)) = v2_temp(1:nbranch,1)';
        tao_1_bin(nk,binIdx(1:nbranch)) = tao_1_temp(1:nbranch,1)';
%     end
