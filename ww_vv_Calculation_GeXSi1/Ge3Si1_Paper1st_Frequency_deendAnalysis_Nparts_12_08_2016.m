...
% % D = 1.32e-45;
% % n=0;
% % % % % Ge: Mingo-2004-NanoLetter: http://pubs.acs.org/doi/pdf/10.1021/nl034721i
% % B = 8.8e-20; 
% % C = 57.6;
% % D = 1.32e-45;
% % n=0;
% % % Ge: -2004-arxiv: https://arxiv.org/ftp/arxiv/papers/1005/1005.2578.pdf
B = 3.35e-19; 
C = 57.6;
D = 2.4e-45;
n=0;
...
% 		   Cpsumnkc(nk) = sum(sum((Cpmodec)));
  ...
  %  Uprocess_scat_inv = bu*(wwmode.*wwmode)./(vmode.*vmode);
    gru =  0.73; theta = 373; % Ge: gamma=0.67(Gauster, JAP, 44,(1973)) % Si: gamma =  1.5; theta = 674; % % 645; % Debye temperature, unit: K; % Glassbrener: Si = 674; % Website: Ge: 373;
%     Uprocess_scat_inv = (h*gamma^2*T/massSi/theta*exp(-theta/3/T))*(wwmode.*wwmode)./(vmode.*vmode);
    dA = -0.6131; dK = -0.3175; % Ge % Si: dA = 1.5849; dK = 0.24; % % % data from Gillet-JHT-2009. 
...
%     massAve = sum(mI)/sum(length(mI))*massSi;
...
% Code_plot_dispersion_k_110_111
