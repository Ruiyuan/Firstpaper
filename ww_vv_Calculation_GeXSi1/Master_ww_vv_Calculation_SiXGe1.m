...
Nk = 20; %120; % 60; %     80; %20; % 100; % 16; %40; %   [60]; %   
...
Ptotal = 400; %  
Pendflag = 93; % 380 ; % 379; % 378; % 377; % 376; % 375; % 374; % 373; % 372;  % 371; % 365; %  364; % 363; % 362; % 361;% 358:360; % 355:357; % [340,353:354]; % [337:339]; % [334:336]; % [100,324:325]; % [158:160]; % [118:120]; % 97:99; % 94:96; % 91:93; % 389:391; % 386:388; % 138:140; % [308:310,179:180,158:160]; % 319:325; % 381:400; % 366:380; % 348:365;% 341:355; % 326:340; % 311:325; % 301:310; % 171:180; % [151:160]; % [76:80,115:120]; % 131:140; % [76:80,115:120]; % 111:120; % 91:100; % 71:80; % 51:60; % 31:40; %  291:300; % 273:280; % 283:290; % 271:280; % 261:270; % 251:260; % 241:250; % 231:240; % 221:230; % 211:220; % 201:210; % 191:200; % 181:190; % 161:180; % 141:160; % 121:140; % 101:120; % 81:100;  % 61:80; % 41:60; % 21:40; % 1:20; % 
mypool = parpool(4);
...
% % % ksumx_4Gillet = zeros(length(kxx),1);1
...
%       fprintf(fileID,'%6d %12.8f %12.8f %12.8f %12.8f %12.8f\n', nk, Cpsumnk(nk),ksumx2(nk), ksumx_NBS(nk), ksumx_3Part(nk),ksumx_4Gillet(nk));
    ...
    fname31 = sprintf('ww_Si%dGe1_Nk_%d_P%d_%d.csv', ncx, N, Pn(end),Ptotal);
    save(fname31,'ww','-ascii');
    fname32 = sprintf('vv_Si%dGe1_Nk_%d_P%d_%d.csv', ncx, N, Pn(end),Ptotal);
