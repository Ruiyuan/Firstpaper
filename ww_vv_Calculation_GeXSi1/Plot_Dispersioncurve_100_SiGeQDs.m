...
% % % C = 137.3; % unit: K. 
% % % D = 1.32e-45; % s^3. % % % N = 30; 
Nk = 120; % 50; % 30; %  % 30; %50; %120; % 120; %    50; %    80; % 30; %  60; %80; %   30; % 120; % 50; %[80]; % 30; %  [10, 20, 30, 40, 50, 80, 100]; % ]; %      
...
Dsuper = Dsuper + Dall(:,(i-1)*naxis*nsupercell+(1:naxis*nsupercell));
  ...
  [eV,SS]=svd(Dsuper);
  wwI = sqrt(diag(SS));
      ww(nk,:) = wwI*sqrt(1/massSi);
if(dispersiononlyflag == 1)
    continue;
end
% % % % %   continue;
...
figure
plot(XX*(kall/kx), ww,'b.')
xlim([0 XX(end)*(kall/kx)])
yyaxis left
