...
vv = zeros(nbranch, N^3);
    ...
    for Pendflag1 = 1:length(Pendflag);
        ...
        fname31 = sprintf('ww_Si%dGe1_Nk_%d_P%d_%d_110.csv', ncx, N, Pendflag1,Ptotal);
        ww(:,kstart:kend) = load(fname31,'-ascii');
        fname32 = sprintf('vv_Si%dGe1_Nk_%d_P%d_%d_110.csv', ncx, N, Pendflag1,Ptotal);
...
if(sum(mI)/sum(length(mI))==1)
    disp('SiXGe1 Calculation');
else
    disp('GeXSi1 Calculation');
end
...
% % D = 1.32e-45;
% % n=0;
% % % Si: Singh-2009-arxiv: https://arxiv.org/ftp/arxiv/papers/1005/1005.2578.pdf
B = 1.73e-19; 
C = 137.39;
D = 1.32e-45;
n=0;
...
% Nk = 50; % 120; %80; % 60; %     80; %20; % 100; % 16; %40; %   [60]; %   
...
ksumx_4GilletcALL = zeros(nbranch, length(kxx));
tau_inv_ALL = zeros(nbranch, length(kxx));
...
for nk = kstart:kstop; % 1: length(kxx); % 
   ...
   if(mod(nk,2000)==0)
   	disp(nk/length(kxx)) 
    		   disp('use kmode_holland for Bulk Si Calculation: ')
...
% 		   Cpsumnkc(nk) = sum(sum((Cpmodec)));
    ...
    kmode_holland = vmode.*vmode.*tot_scat.*Cpmode; % 

       
   %  Uprocess_scat_inv = bu*(wwmode.*wwmode)./(vmode.*vmode);
    gru = 0.45; theta = 674; % 645; % Debye temperature, unit: K; % Glassbrener: Si = 674; % Website: Ge: 373;
...
% % %     ksumx_4Gillet(nk) = sum(sum(kmode_holland4_Gillet))*8/V;
    ...
    % Debye temperature, unit: K; % Glassbrener: Si = 674; % Website: Ge: 373;
    massAve = sum(mI)/sum(length(mI));
		   Uprocess_scat_invc = (h*gru^2*T/massAve/theta*exp(-theta/3/T))*(wwmodec.*wwmodec)./(vmode.*vmode);
% 		   tot_scat_inv_Gilletc = Uprocess_scat_invc + inc_scat_inv;
           % % % 
              if(mod(nk,2000)==0)
           disp('Umklapp eqn from Bulk Si - Davis-2011 Paper')
              end
           clear tot_scat_inv_Gilletc
		   tot_scat_inv_Gilletc = umk_scat_inv + inc_scat_inv;
           ...
           sum_umk_inv = sum_umk_inv + sum(sum(umk_scat_inv));
           sum_inc_inv =  sum_inc_inv + sum(sum(inc_scat_inv));
           sum_tot_inv = sum_tot_inv + sum(sum(tot_scat_inv_Gilletc));
         
           %            kmode_holland4_Gilletc = kmode_holland; % 
           ksumx_4Gilletc(nk) = sum(sum(kmode_holland4_Gilletc))*8/V; % % tao_1; 
           ksumx_4GilletcALL(:,nk) = kmode_holland4_Gilletc.*8/V;
           tau_inv_ALL(:,nk) = tot_scat_inv_Gilletc; % 
           
            if(sqrt(kx^2+ky^2+kz^2)>(XX(end))*sqrt(2)) %1); % 
%                continue;
...
[k_Gilletc, k_SBZ, k_110SBZ, k_110All, k_111SBZ, k_111All]




[sum_inc_inv, sum_umk_inv, sum_tot_inv]

[sum_inc_inv, sum_umk_inv, sum_tot_inv]./(N^3*nbranch)
